...
whichB = np.arange(27, 63)      # Indices 27 to 62 for the next 36 atoms (Compound A)
...
############################################################
if os.path.isfile('et.npy') and os.path.isfile('na.npy') and os.path.isfile('ad.npy'):
    et = np.load('et.npy')
    na = np.load('na.npy')
    ad = np.load('ad.npy')
    print(et.shape)
    ...
    output_data = np.column_stack((np.arange(len(et)), et, np.cumsum(na), np.cumsum(ad)))
    header = "Time [fs]    ET   Cumulative NA    Cumulative AD"
    np.savetxt('electron_localization.dat', output_data, header=header, fmt='%12.6f')
    ...
    ax.plot(et[:, 2],       ls='-', lw=1.0, color='red',    label=r'CBM+2@Zn$_2$S$_2$')
    ax.plot(et[:, 1],       ls='-', lw=1.0, color='purple',    label=r'CBM+1@Zn$_2$S$_2$')
    ax.plot(et[:, 0],       ls='-', lw=1.0, color='darkgreen', label=r'CBM@CdCl$_2$')
    ...
    ax.set_xlabel('Time (fs)', fontsize='medium', labelpad=5)
    ax.set_ylabel('Electron Localization', fontsize='medium', labelpad=5)

    plt.tight_layout(pad=0.5)
    plt.savefig('kspat_electron.png', dpi=1080)
